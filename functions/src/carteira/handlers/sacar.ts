...
import { atualizarSaldo, buscarCarteira, registrarOperacao } from "../repositories/carteiraRepository";
...
export const sacar = onCall(
    async (request: CallableRequest<AtualizarSaldoInput>) => {
        ...
        if(valor > carteira.saldo){
            throw new HttpsError("invalid-argument", 'Saldo insuficiente para realizar o saque')
        }
